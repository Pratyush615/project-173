...
// call the handleMarkerLost function
  ...
  handleRatings: async function(toy) {
    ...
    var users;
    users <= 9 ? (users = `T0${uid}`): (`T${users}`)

    var orderSummary = await this.getToys(users)
    ...
    if(currentOrders.length>0 && currentOrders == dish.id){
      ...
      saveRatingButton.addEventListener("click", () => {
        ...
        var feedback = document.getElementById('feed-back-input').value
        firebase
          .firestore()
          .collection("toys")
          .doc(toy.id)
      ...
      swal({
        icon:'warning',
        title:'Oops!',
        text:'No toy ordered to give ratings',
